...
import { getUsers, getTasks, getTaskAccessEntries, UserIds } from './seed-data';
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    logger.error('[SEED] ❌ Seeding eșuat — rollback efectuat', { error: (err as Error).message, stack: (err as Error).stack });
    process.exit(1);
  } finally {
    client.release();
    await pool.end();
