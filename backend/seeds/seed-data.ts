 ...
 */
const hashPassword = (plaintext: string): Promise<string> => bcrypt.hash(plaintext, SALT_ROUNDS);

/**
 * Helper — returns a Date offset from now.
 * @param hours - positive = future, negative = past
 */
 ...
 */
const getUsers = async (): Promise<SeedUser[]> => {
 ...
 */
const getTasks = (userIds: UserIds): SeedTask[] => [
  ...
  },
  {
    ...
    description: 'Rulare EXPLAIN pe query-urile din task.model.ts și adăugare indexuri dacă e nevoie.',
    status: 'todo',
    visibility: 'personal',
    priority: 'low',
 ...
 */
const getTaskAccessEntries = (): TaskAccessEntry[] => [
...
export { getUsers, getTasks, getTaskAccessEntries };
export type { SeedUser, SeedTask, TaskAccessEntry, UserIds };
