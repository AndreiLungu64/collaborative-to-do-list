 ...
 * Utilizare: npx tsx run_migration.ts [fisier.sql]
...
async function runMigration(filePath: string): Promise<void> {
  ...
  try {
    ...
    res.rows.forEach((row: { table_name: string }) => console.log(`   - ${row.table_name}`));
    
  } catch (err) {
    console.error('❌ Eroare la migratie:', (err as Error).message);
    process.exit(1);
  } finally {
    await client.end();
    console.log('\n🔌 Conexiune inchisa.');
